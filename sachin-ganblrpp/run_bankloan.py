...
df = pd.read_csv("katabatic/models/bankloan.csv")
target_col = "Personal Loan"
X = df.drop(columns=["ID", "ZIP Code", target_col])  # Drop non-features
y = df[target_col]
...
X[categorical_cols] = X[categorical_cols].astype(str)  # Ensure encoding works
...
results_df.to_csv("bankloan_tstr_results.csv", index=False)
print("\n✅ Results saved to bankloan_tstr_results.csv")
...
plt.title("Average TSTR Accuracy over 10 Runs - Bank Loan Dataset")
plt.ylabel("Accuracy")
plt.xticks(rotation=15)
plt.grid(True, axis='y')
plt.tight_layout()
plt.savefig("bankloan_tstr_accuracy_summary.png")
plt.show()
