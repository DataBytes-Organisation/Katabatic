...
from katabatic.katabatic_spi import KatabaticModelSPI  #to integrate with Katabatic
...
class CtganAdapter(KatabaticModelSPI):
    ...
    def fit(self, X, y=None):
        ...
        if y is not None:
            ...
            data = pd.concat([X.reset_index(drop=True), y.reset_index(drop=True).rename(y_name)], axis=1)
            self.target_column = y_name
        else:
            data = X
            self.target_column = None
