    ...
    def __init__(self, raw_df: pd.DataFrame, categorical: list, log:list, mixed:dict, general:list, non_categorical:list, integer:list, type:dict, test_ratio:float):
        ...
        if problem:
            ...
            if problem=="Classification":
                X_train_real, X_test_real, y_train_real, y_test_real = model_selection.train_test_split(X_real ,y_real, test_size=test_ratio, stratify=y_real,random_state=42, shuffle=True)
                self.x_train = X_train_real
                self.y_train = y_train_real
                self.x_test = X_test_real
                self.y_test = y_test_real
            else:
                X_train_real, X_test_real, y_train_real, y_test_real = model_selection.train_test_split(X_real ,y_real, test_size=test_ratio,random_state=42, shuffle=True)
                self.x_train = X_train_real
                self.y_train = y_train_real
                self.x_test = X_test_real
                self.y_test = y_test_real
