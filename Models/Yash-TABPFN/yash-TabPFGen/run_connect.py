...
# =======================
file_path = r'C:\Users\Manthan Goyal\Desktop\Team-Project\TabPFGen\datasets\connect-4.arff'
print("🔹 Loading Connect-4 dataset...")
data, meta = arff.loadarff(file_path)
df = pd.DataFrame(data)

# Limit to 10,000 samples
print("✂️ Limiting dataset to 10,000 samples (random selection)...")
...
# Replace values
X = df.drop(columns=['class']).replace({'x': 1, 'o': 2, 'b': 0})
...
classifiers = {
    ...
    'XG BOOST': XGBClassifier(eval_metric='logloss', num_class=len(np.unique(y))),
...
for repeat in range(1, 4):
    for fold, (train_index, test_index) in enumerate(kf.split(X), 1):
        ...
        real_classes = set(train_y)
        synth_classes = set(y_synth)
        missing_classes = real_classes - synth_classes

        if missing_classes:
            print(f"⚠️ Injecting missing classes: {missing_classes}")
            for cls in missing_classes:
                samples_to_add = train_X[train_y == cls]
                if len(samples_to_add) > 0:
                    sample = samples_to_add.sample(1, random_state=42)  # Just one instance
                    X_synth = np.vstack((X_synth, sample.to_numpy()))
                    y_synth = np.hstack((y_synth, [cls]))
...
# =======================
print("\n📊 Final Cross-Validation Summary for Connect-4 Dataset:\n")
