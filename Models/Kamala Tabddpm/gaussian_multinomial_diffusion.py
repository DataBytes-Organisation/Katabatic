...
class GaussianMultinomialDiffusion(torch.nn.Module):
    def __init__(
        ...
        # Handle empty num_classes array
        if len(num_classes) > 0:
            ...
            # Create empty tensors for the case with no categorical features
    ...
    def mixed_loss(self, x, out_dict):
        ...
        if x_cat.shape[1] > 0:
            # Convert num_classes elements to int from numpy array if needed
            num_classes_int = [int(nc) if isinstance(nc, np.ndarray) else nc for nc in self.num_classes]
            log_x_cat = index_to_log_onehot(x_cat.long(), num_classes_int)
    ...
    def sample_ddim(self, num_samples, y_dist):
        ...
        # Use len to check if array is empty first
        has_cat = len(self.num_classes) > 0 and self.num_classes[0] != 0
        ...
        # Check if y_dist is valid before using it
        if y_dist is not None and isinstance(y_dist, torch.Tensor) and y_dist.numel() > 0:
            y = torch.multinomial(
                y_dist,
                num_samples=b,
                replacement=True
        ...
        # Handle categorical data differently based on has_cat
        if has_cat:
            z_ohe = torch.exp(log_z).round()
            z_cat = ohe_to_categories(z_ohe, self.num_classes)
            sample = torch.cat([z_norm, z_cat], dim=1).cpu()
        else:
            # If no categorical features, just return numerical features
            sample = z_norm.cpu()
    ...
    def sample(self, num_samples, y_dist):
        ...
        y = None
        if y_dist is not None and y_dist.shape[0] > 0:
            y = torch.multinomial(
                y_dist,
                num_samples=b,
                replacement=True
        ...
        print()
        z_ohe = torch.exp(log_z).round() if has_cat else torch.zeros((b, 0), device=device)
        z_cat = log_z
        if has_cat:
            z_cat = ohe_to_categories(z_ohe, self.num_classes)
        sample = torch.cat([z_norm, z_cat], dim=1).cpu()
    ...
    def sample_all(self, num_samples, batch_size, y_dist, ddim=False):
        ...
        if len(all_y) > 0:
            ...
            y_gen = torch.zeros(num_samples)  # Default values if no target was generated
