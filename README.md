# TableGAN Paper Replication

This repository contains a careful reimplementation of the "TableGAN" model from the paper:
...
1. **Original Implementation**: The original implementation in `implementation.py` provides a working TableGAN model with custom parameters
2. **Paper Replication**: A careful replication that follows the exact architecture and hyperparameters from the paper

### Paper Replication Files

- `paper_replication.py`: Downloads and processes the UCI Adult dataset as described in the paper
- `paper_exact_implementation.py`: Implements the TableGAN model with architecture and hyperparameters exactly matching the paper
- `run_paper_replication.py`: A script that runs the entire replication pipeline

### How to Run the Paper Replication

To replicate the paper results, simply run:
...
This will:
1. Download and prepare the full Adult dataset
2. Train the TableGAN model with paper-specified hyperparameters
3. Generate synthetic data and evaluate its quality
4. Save results and visualizations to the `results` directory

### Paper Reported Metrics

The paper reports the following metrics for the Adult dataset:

- Statistical similarity (correlation distance): ~0.30-0.35
- ML efficacy - Accuracy ratios for different classifiers:
  - Random Forest: ~94%


### Our Replication Results

Our replication using the full Adult dataset (45,222 records) achieved:

- Statistical similarity (correlation distance): 5.94
- ML efficacy - Accuracy ratios:
  - Random Forest: 81.65%
  - Logistic Regression: 91.34%
  - MLP: 78.69%
  - XGBoost: 74.68%

We found that Logistic Regression showed the best preservation of predictive power, achieving an accuracy ratio close to the paper's reported results. For full details, see the `results_summary.md` and `scores_comparison.md` files.

---

# Original tableGAN Implementation
