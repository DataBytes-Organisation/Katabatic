...
def ohe_to_categories(ohe, K):
    ...
    # Make sure both tensors being concatenated have the same dimension
    zeros = torch.zeros(1, device=K.device)
    indices = torch.cat([zeros, K.cumsum(dim=0)], dim=0).int().tolist()
...
def index_to_log_onehot(x, num_classes):
    ...
    for i in range(len(num_classes)):
        ...
        n_classes = int(num_classes[i]) if isinstance(num_classes[i], np.ndarray) else num_classes[i]
        onehots.append(F.one_hot(x[:, i], n_classes))
