...
def generate_visualizations():
    try:
        ...
        plt.savefig(os.path.join(VISUALIZATION_DIR, "real_data_correlation_heatmap.png"))
        plt.close()

        plt.figure(figsize=(10, 8))
        sns.heatmap(synthetic_corr, annot=False, cmap='coolwarm', cbar=True)
        plt.title("Correlation Heatmap - Synthetic Data")
        plt.savefig(os.path.join(VISUALIZATION_DIR, "synthetic_data_correlation_heatmap.png"))
        plt.close()
